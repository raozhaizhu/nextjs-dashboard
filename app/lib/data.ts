...
export async function fetchCardData() {
    try {
        ...
        // 执行查询
        const [invoiceData, customerData, invoiceStatusData] = await Promise.all([
            invoiceCountPromise,
        ...
        const numberOfCustomers = Number(customerData[0]?.count ?? '0');
        const totalPaidInvoices = formatCurrency(invoiceStatusData[0]?.paid ?? '0');
        const totalPendingInvoices = formatCurrency(invoiceStatusData[0]?.pending ?? '0');
